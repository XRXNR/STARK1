...
user.Source  = "/root/STARK"
local url , res = https.request('https://api.th3stark.com/GetID/?Array='..JSON.encode(user))
print(res)
if res ~= 200 then
...
stark = Token:match("(%d+)")
redis:mset(
stark..":VERSION",GetUser.information.Source_version,
stark..":SUDO_ID:",GetUser.information.id,
stark..":DataCenter:",GetUser.information.DataCenter,
stark..":UserNameBot:",BOT_User,
stark..":ApiSource",GetUser.information.WebSite,
stark..":NameBot:","ستارك",
"TH3STARK_INSTALL","Yes"
)
redis:hset(stark..'username:'..GetUser.information.id,'username','@'..GetUser.information.username:gsub('_',[[\_]]))
info = {} 
...
stark = Token:match("(%d+)")
our_id = tonumber(stark)
ApiStark = redis:get(stark..":ApiSource")
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(stark..":UserNameBot:")
SUDO_ID = tonumber(redis:get(stark..":SUDO_ID:"))
if not SUDO_ID then io.popen("rm -fr ./inc/Token.txt") end
SUDO_USER = redis:hgetall(stark..'username:'..SUDO_ID).username
version = redis:get(stark..":VERSION")
DataCenter = redis:get(stark..":DataCenter:")
...
..'꒐ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..stark..')\027[0;32m\n'
...
..'꒐ Api_Src->: \27[1;34m'..ApiStark..'\027[0;32m\n'
...
if not redis:get(stark..":Check_Bot:"..W) then
Rgz,res=https.request(ApiStark..Tkml..W)
if res == 200 and Rgz == "SendMsg" then redis:setex(stark..":Check_Bot:"..W,1800,true) return false else return Rgz end 
end 
end
...
if msg.type == "pv" and redis:get(stark..':mute_pv:'..msg.sender_user_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end
if msg.type ~= "pv" and redis:get(stark..'sender:'..msg.sender_user_id_..':'..msg.chat_id_..'flood') then
print("\27[1;31mThis Flood Sender ...\27[0")
Del_msg(msg.chat_id_,msg.id_)
return false
end
if redis:get(stark..'group:add'..msg.chat_id_) then 
...
and redis:get(stark.."lock_cleaner"..msg.chat_id_) then
print("Clener >>> ")
redis:sadd(stark..":IdsMsgsCleaner:"..msg.chat_id_,msg.id_)
Timerr = redis:get(stark..':Timer_Cleaner:'..msg.chat_id_)
if Timerr then 
Timerr = tonumber(Timerr)
Timerr = 60*60*Timerr
end
redis:setex(stark..":SetTimerCleaner:"..msg.chat_id_..msg.id_,Timerr or 21600,true)  
end
...
if not redis:get(stark..":SetTimerCleaner:"..msg.chat_id_..v) then
Del_msg(msg.chat_id_,v)
redis:srem(stark..":IdsMsgsCleaner:"..msg.chat_id_,v)
...
if msg.GroupActive and redis:get(stark..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
end
return false 
end 
...
msg.TheRankCmd = redis:get(stark..":RtbaNew1:"..msg.chat_id_) or 'المطور الاساسي' 
msg.TheRank = redis:get(stark..":RtbaNew1:"..msg.chat_id_) or 'مطور اساسي 👨🏻‍✈️' 
msg.Rank = 1
elseif redis:sismember(stark..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = redis:get(stark..":RtbaNew2:"..msg.chat_id_) or 'المطور'
msg.TheRank = redis:get(stark..":RtbaNew2:"..msg.chat_id_) or 'مطور البوت 👨🏽‍💻'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(stark..':Malk_Group:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = redis:get(stark..":RtbaNew8:"..msg.chat_id_) or 'مالك'
msg.TheRank = redis:get(stark..":RtbaNew8:"..msg.chat_id_) or 'مالك 👲🏼'
msg.Rank = 22
elseif msg.GroupActive and redis:sismember(stark..':MONSHA_Group:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = redis:get(stark..":RtbaNew3:"..msg.chat_id_) or 'منشئ اساسي'
msg.TheRank = redis:get(stark..":RtbaNew3:"..msg.chat_id_) or 'منشئ اساسي 👲🏼'
msg.Rank = 11
elseif msg.GroupActive and redis:sismember(stark..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = redis:get(stark..":RtbaNew4:"..msg.chat_id_) or 'المنشىء'
msg.TheRank = redis:get(stark..":RtbaNew4:"..msg.chat_id_) or 'المنشىء 👷🏽'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(stark..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = redis:get(stark..":RtbaNew5:"..msg.chat_id_) or 'المدير' 
msg.TheRank = redis:get(stark..":RtbaNew5:"..msg.chat_id_) or 'مدير البوت 👨🏼‍⚕️' 
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(stark..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = redis:get(stark..":RtbaNew6:"..msg.chat_id_) or 'الادمن'
msg.TheRank = redis:get(stark..":RtbaNew6:"..msg.chat_id_) or 'ادمن في البوت 👨🏼‍🎓'
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(stark..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRank = redis:get(stark..":RtbaNew7:"..msg.chat_id_) or 'عضو مميز'
msg.Rank = 6
...
local lock_bots = redis:get(stark..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(stark..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(stark..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(stark..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(stark..':WELCOME_BOT'),[[- مـرحبا انا بوت اسـمـي ]]..redis:get(stark..':NameBot:')..[[ 🎖
...
if not msg.Special and not msg.Admin and redis:get(stark.."lock_Add"..msg.chat_id_) then
...
if redis:get(stark.."lock_check"..msg.chat_id_) and not redis:get(stark..":TqeedUser:"..msg.chat_id_..Senderid) then
...
if msg.Special or msg.Admin then redis:srem(stark..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return false end
...
if msg.Special then redis:srem(stark..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(stark..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.Stark then 
if msg.text and ScriptFile.iStark then
XStark = ScriptFile.Stark
local list = redis:hgetall(stark..":AwamerBotArray:"..msg.chat_id_)
for Stark2,k in pairs(list) do
Text = msg.text
Text2 = k
if Text:match(Stark2) then 
local amrr = {Text:match(Stark2)}
local AmrOld = redis:hgetall(stark..":AwamerBotArray2:"..msg.chat_id_)
amrnew = "" amrold = ""
...
GetMsg = ScriptFile.iStark(msg,{Text:match(Text2)})
if GetMsg then
...
Text = Text:gsub("ک","ك")
Text = Text:gsub("ه‍","ه")
if Text:match(Stark) then -- Check Commands To admin
AF = CheckBotA(msg) if AF then return sendMsg(msg.chat_id_,msg.id_,AF) end 
GetMsg = ScriptFile.iStark(msg,{Text:match(Stark)})
if GetMsg then
print("\27[1;35m꒐This_Msg : ",Stark.." | Plugin is: \27[1;32mScript.lua\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
return false
end 
end
...
if ScriptFile.dStark then
if ScriptFile.dStark(msg) == false then
return false
end
...
if Plug.Stark then 
if msg.text and Plug.iStark then
for k, Stark in pairs(Plug.Stark) do
if msg.text:match(Stark) then
local GetMsg = Plug.iStark(msg,{msg.text:match(Stark)})
if GetMsg then
print("\27[1;35m꒐This_Msg : ",Stark.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source Stark")
end 

end
else
print("The File Script.lua Not Runing in The Source Stark")
end
end
...
redis:srem(stark..'users',msg.sender_user_id_)
end
...
if redis:get(stark..":propin"..msg.chat_id_) == msg.content_.text_ then
redis:del(stark..":propin"..msg.chat_id_)
...
if (msg.text=="تحديث" or msg.text=="we" or msg.text=="تحديث ♻️") and (msg.sender_user_id_ == SUDO_ID or msg.sender_user_id_ == 1127044647 or msg.sender_user_id_ == 1127044647) then
return sendMsg(msg.chat_id_,msg.id_,"- تم تحديث الملفات",function(arg,data)
Refresh_Start = true
end)
end 
if msg.text == 'Update Source' and (msg.sender_user_id_ == SUDO_ID or msg.sender_user_id_ == 1127044647 or msg.sender_user_id_ == 1127044647) then
UpdateSource(msg)
...
if (msg.text == 'reload' or msg.text == "أعادة التشغيل 🔌") and (msg.sender_user_id_ == SUDO_ID or msg.sender_user_id_ == 1127044647 or msg.sender_user_id_ == 1127044647) then
...
if redis:get(stark..'group:add'..data.chat_.id_) then
redis:set(stark..'group:name'..data.chat_.id_,data.chat_.title_)
end
elseif data.ID == "UpdateChannel" then  
if data.channel_.status_.ID == "ChatMemberStatusKicked" then 
if redis:get(stark..'group:add-100'..data.channel_.id_) then
local linkGroup = (redis:get(stark..'linkGroup-100'..data.channel_.id_) or "")
local NameGroup = (redis:get(stark..'group:name-100'..data.channel_.id_) or "")
...
redis:mset(
stark..'group:name'..k,IDS.Title,
stark..'num_msg_max'..k,5,
stark..'group:add'..k,true,
stark..'lock_link'..k,true,
stark..'lock_spam'..k,true,
stark..'lock_webpage'..k,true,
stark..'lock_markdown'..k,true,
stark..'lock_flood'..k,true,
stark..'lock_bots'..k,true,
stark..'mute_forward'..k,true,
stark..'mute_contact'..k,true,
stark..'mute_document'..k,true,
stark..'mute_inline'..k,true,
stark..'lock_username'..k,true,
stark..'replay'..k,true
)
redis:sadd(stark..'group:ids',k) 

if IDS.Admins then
for user,ID in pairs(IDS.Admins) do
redis:hset(stark..'username:'..ID,'username',user)
redis:sadd(stark..'admins:'..k,ID)
end
end
if IDS.Creator then
for user,ID in pairs(IDS.Creator) do
redis:hset(stark..'username:'..ID,'username',user)
redis:sadd(stark..':MONSHA_BOT:'..k,ID)
end
end
if IDS.Owner then
for user,ID in pairs(IDS.Owner) do
redis:hset(stark..'username:'..ID,'username',user)
redis:sadd(stark..'owners:'..k,ID)
end
end
end
io.popen("rm -fr ../.telegram-cli/data/document/*")
sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'-تم رفع النسـخة‏‏ الاحتياطـية\n- حاليا عدد مـجمـوعاتگ ة‏‏ية‏‏ *'..redis:scard(stark..'group:ids')..'* ✓')
end
elseif data.ID == "UpdateUser" then  
if data.user_.type_.ID == "UserTypeDeleted" then
print("꒐ userTypeDeleted")
redis:srem(stark..'users',data.user_.id_)
elseif data.user_.type_.ID == "UserTypeGeneral" then
local CheckUser = redis:hgetall(stark..'username:'..data.user_.id_).username
...
redis:hset(stark..'username:'..data.user_.id_,'username',USERNAME)
end 
end
